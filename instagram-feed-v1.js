...
(function(){
  ...
  function sectionHTML(shop,posts){
    if(!posts||!posts.length)return'';
    const handle=username(shop.instagram);
    return `<section class="instagram-feed-section" data-instagram-feed><div class="instagram-feed-head"><div class="instagram-feed-title-wrap"><h2 class="instagram-feed-title"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.4" cy="6.7" r="1" fill="currentColor" stroke="none"/></svg>Instagram</h2>${handle?`<p class="instagram-feed-handle">@${esc(handle)}</p>`:''}</div><a class="instagram-feed-more" href="${esc(shop.instagram)}" target="_blank" rel="noopener">전체보기 →</a></div><div class="instagram-feed-grid">${posts.slice(0,3).map(p=>`<a class="instagram-feed-item" href="${esc(p.permalink||shop.instagram)}" target="_blank" rel="noopener" aria-label="Instagram 게시물 보기"><img src="${esc(p.image||p.thumbnail_url||p.media_url)}" alt="${esc(shop.name)} Instagram 게시물" loading="lazy" decoding="async" referrerpolicy="no-referrer"></a>`).join('')}</div></section>`;
  }
  async function load(shop){
    if(!shop||!ELIGIBLE.has(shop.id)||!shop.instagram||!API)return;
    try{
      ...
      const temp=document.createElement('div');temp.innerHTML=sectionHTML(shop,valid);const section=temp.firstElementChild;
