...
(function(){
  ...
  function balanceMasonry(section){const cols=[...section.querySelectorAll('.instagram-feed-column')],pool=section.querySelector('.instagram-feed-pool');if(cols.length!==2||!pool)return;const items=[...pool.children];let heights=[0,0],pending=items.length;const place=()=>{items.forEach((item,i)=>{const img=item.querySelector('img'),ratio=img&&img.naturalWidth?img.naturalHeight/img.naturalWidth:1.25,target=heights[0]<=heights[1]?0:1;cols[target].appendChild(item);heights[target]+=ratio});pool.remove()};if(!pending){pool.remove();return}const ready=()=>{if(--pending===0)place()};items.forEach(item=>{const img=item.querySelector('img');if(!img){ready();return}if(img.complete){ready();return}img.addEventListener('load',ready,{once:true});img.addEventListener('error',ready,{once:true})})}
