...
(function(){
  ...
  function markerIcon(selected=false){const m=markerMetrics();return {content:markerHTML(selected),anchor:new naver.maps.Point(Math.round(m.markerW/2),m.markerH)}}
  function setSelected(marker,selected){if(!marker)return;try{marker.setIcon(markerIcon(selected))}catch(e){}try{marker.setZIndex(selected?500:100)}catch(e){}}
  function clearSelected(closeInfo=false){if(selectedMarker){setSelected(selectedMarker,false);selectedMarker=null}if(closeInfo){try{if(activeInfoWindow)activeInfoWindow.close()}catch(e){}}}
  function refreshMarkerSizes(){try{SHOPS.forEach(s=>{const marker=markerById.get(s.id);if(marker)setSelected(marker,marker===selectedMarker)})}catch(e){}}
  ...
  function bindMarker(s,marker){if(!marker||bound.has(marker))return;setSelected(marker,false);try{naver.maps.Event.clearInstanceListeners(marker)}catch(e){}naver.maps.Event.addListener(marker,'click',()=>{if(selectedMarker===marker){clearSelected(true);return}if(selectedMarker)setSelected(selectedMarker,false);selectedMarker=marker;setSelected(marker,true);try{if(activeInfoWindow)activeInfoWindow.close()}catch(e){}const iw=new naver.maps.InfoWindow({content:popupHTML(s),borderWidth:0,backgroundColor:'transparent',anchorSize:new naver.maps.Size(0,0),anchorSkew:false,pixelOffset:new naver.maps.Point(0,-20)});iw.open(naverMap,marker);try{activeInfoWindow=iw}catch(e){}});bound.add(marker)}
  ...
  ensureMarkerStyles();let tries=0;const timer=setInterval(()=>{tries++;try{SHOPS.forEach(s=>bindMarker(s,markerById.get(s.id)));bindMapEvents();if(markerById.size>=SHOPS.length||tries>60)clearInterval(timer)}catch(e){if(tries>60)clearInterval(timer)}},250)
})();