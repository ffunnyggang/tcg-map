...
export async function signUp(email:string,password:string,nickname:string){const {data,error}=await supabase.auth.signUp({email,password,options:{data:{nickname:nickname.trim()}}});if(error)throw error;if(data.user&&data.session){const {error:e}=await supabase.from('profiles').upsert({user_id:data.user.id,nickname:nickname.trim()||null});if(e)throw e}return data}
